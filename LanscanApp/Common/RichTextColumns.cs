...
namespace LanscanApp.Common
{
    ...
    public sealed class RichTextColumns : Panel
    {
        ...
        /// RichTextBlock child.
        /// </summary>
        private List<RichTextBlockOverflow> m_overflowColumns = null;
        ...
        protected override Size MeasureOverride(Size availableSize)
        {
            ...
            // done yet
            if (this.m_overflowColumns == null)
            {
                Children.Add(this.RichTextContent);
                this.m_overflowColumns = new List<RichTextBlockOverflow>();
            }
            ...
            while (hasOverflow && maxWidth < availableSize.Width && this.ColumnTemplate != null)
            {
                ...
                if (this.m_overflowColumns.Count > overflowIndex)
                {
                    overflow = this.m_overflowColumns[overflowIndex];
                }
                else
                {
                    overflow = (RichTextBlockOverflow)this.ColumnTemplate.LoadContent();
                    this.m_overflowColumns.Add(overflow);
                    ...
                    {
                        this.m_overflowColumns[overflowIndex - 1].OverflowContentTarget = overflow;
                    }
                }
            ...
            if (this.m_overflowColumns.Count > overflowIndex)
            {
                ...
                {
                    this.m_overflowColumns[overflowIndex - 1].OverflowContentTarget = null;
                }
                while (this.m_overflowColumns.Count > overflowIndex)
                {
                    this.m_overflowColumns.RemoveAt(overflowIndex);
